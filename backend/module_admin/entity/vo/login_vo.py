...
class UserLogin(BaseModel):
    ...
    username: str = Field(description='用户名称', alias='user_name')
    password: str = Field(description='用户密码')
    code: Optional[str] = Field(default=None, description='验证码')
    uuid: Optional[str] = Field(default=None, description='会话编号')
    login_info: Optional[dict] = Field(default=None, description='登录信息，前端无需传递', alias='loginInfo')
    captcha_enabled: Optional[bool] = Field(default=None, description='是否启用验证码，前端无需传递', alias='captchaEnabled')
