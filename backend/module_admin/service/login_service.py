...
class LoginService:
    ...
    async def authenticate_user(
        ...
        account_lock = await request.app.state.redis.get(
            f'{RedisInitKeyConfig.ACCOUNT_LOCK.key}:{login_user.username}'
        )
        ...
        user = await login_by_account(query_db, login_user.username)
        if not user:
        ...
        if not PwdUtil.verify_password(login_user.password, user[0].password):
            cache_password_error_count = await request.app.state.redis.get(
                f'{RedisInitKeyConfig.PASSWORD_ERROR_COUNT.key}:{login_user.username}'
            )
        ...
        await request.app.state.redis.delete(f'{RedisInitKeyConfig.PASSWORD_ERROR_COUNT.key}:{login_user.username}')
        return user
    ...
    async def get_current_user(
        ...
        if token == redis_token:
            ...
            is_default_modify_pwd = await cls.__init_password_is_modify(
                request, getattr(query_user.get('user_basic_info'), 'pwd_update_date', None)
            )
