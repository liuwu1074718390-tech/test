<template>
	<div class="gz-upload-file">
		...
		>
			<!-- 上传按钮 -->
			<el-button ref="uploadBtn" :type="btnType" :title="tipsTxt" :text="isTextBtn">
		...
		<!-- <template v-if="previewImg">
			<div class="flex-start-horizontal">
				<p class="p-5 bor">
					<img style="width: 86px" src="@/assets/icons/other/license.png" alt="" />
				</p>
				<a class="color-link color-pr ml-8" @click="showImgPreview = true" target="_blank">预览</a>
				<teleport to="body">
					<el-image-viewer
						v-if="showImgPreview"
						:z-index="9999999"
...
const reSelectFile = () => {
	// nextTick(() => {
	const fileUploadRef = proxy.$refs.fileUpload;
	if (fileUploadRef) {
		const inputElement = fileUploadRef.$el.querySelector('.el-upload__input');
		if (inputElement) {
			inputElement.click();
		} else {
		...
		console.error('fileUpload ref is not available');
	}
	// });
};
defineExpose({
