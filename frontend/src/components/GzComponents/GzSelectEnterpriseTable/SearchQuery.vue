<template>
	<div class="search-q">
		<el-form class="gz-form-search" :model="queryParams" ref="queryRef" :inline="true">
			...
			<el-form-item label="注册地" prop="reAreaId">
				<GzAreaSelect
					ref="selectAreaRef"
			...
			<el-form-item v-if="props.type == 'my'" label="来源" prop="supplySource">
				<el-select
					v-model="queryParams.supplySource"
					placeholder="请选择"
					clearable
					style="width: 160px"
					filterable
					@change="handleQuery"
				>
					<el-option v-for="dict in supply_source" :key="dict.value" :label="dict.label" :value="dict.value" /> </el-select
			></el-form-item>
			<el-form-item v-if="props.type == 'other'" label="企业身份" prop="supplySource">
				<el-select
...
<script setup>
import { ref, reactive, toRefs, watch, getCurrentInstance, onBeforeMount, defineAsyncComponent } from 'vue';
...
function reset(queryRef) {
	...
	queryParams.value.reAreaId = null;
