...
const hideComponent_ = ref([]); //重复命名，反面教材。修改后添加_
const expression_ = ref(''); //重复命名，反面教材。修改后添加_
...
watch(expression_, () => resolveExp());
function shouldHide(key) {
	return !(hideComponent_.value && hideComponent_.value.includes(key));
}
function resolveExp() {
	// 反解析 表达式
	if (expression_.value) {
		const arr = expression_.value.split(/\s+/);
		if (arr.length >= 6) {
			//6 位以上是合法表达式
			let obj = {
				second: arr[0],
...
onMounted(() => {
	expression_.value = props.expression;
	hideComponent_.value = props.hideComponent;
});
</script>
