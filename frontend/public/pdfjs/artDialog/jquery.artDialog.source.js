...
(function ($, window, undefined) {
	...
	artDialog.fn = artDialog.prototype = {
		...
		follow: function (elem) {
			...
			setLeft =
				setLeft < dl ? left : setLeft + wrapWidth > winWidth && left - wrapWidth > dl ? left - wrapWidth + width : setLeft;
			...
			? function () {
					...
					for (; i < list.length; i++) {
						...
						if (png) {
							...
							runtimeStyle.filter =
								'progid:DXImageTransform.Microsoft.' + "AlphaImageLoader(src='" + pngPath + "',sizingMethod='crop')";
						}
					}
				}
			: $.noop,
			...
			? function () {
					...
					if (iframe) {
						...
						iframe.style.cssText =
							'position:absolute;z-index:-1;left:0;top:0;' + 'filter:alpha(opacity=0);width:' + width + ';height:' + height;
					}
				}
			: $.noop,
